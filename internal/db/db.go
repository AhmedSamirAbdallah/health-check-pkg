...
func InitDB(DBURI string) {
	initOnce.Do(func() {
		cliebtOptions := options.Client().ApplyURI(DBURI)
		...
		sharedClient = client
		log.Println("DBConnection : DB connection successful.")
	})
}
...
func CheckReadOnDB(databaseName string, collectionName string) bool {
	collection := sharedClient.Database(databaseName).Collection(collectionName)
...
func CheckWriteOnDB(databaseName string, collectionName string) bool {
	collection := sharedClient.Database(databaseName).Collection(collectionName)
